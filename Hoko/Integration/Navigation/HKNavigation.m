...
#import "HKNavigation.h"

@implementation HKNavigation

#pragma mark - Singleton
+ (instancetype)sharedInstance
{
  static HKNavigation *_sharedInstance = nil;
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    _sharedInstance = [HKNavigation new];
  });
...
{
  [HKNavigation sharedInstance].applicationDelegate.window.rootViewController = viewController;
}
...
+ (void)pushViewController:(UIViewController *)viewController animated:(BOOL)animated replace:(BOOL)replace
{
  ...
  if([viewController isKindOfClass:[UINavigationController class]])
    ...
    UINavigationController *navigationController = [HKNavigation sharedInstance].currentNavigationViewController;
    if (navigationController) {
      ...
      [HKNavigation sharedInstance].applicationDelegate.window.rootViewController = navigationController;
    }
  }
}
...
+ (void)presentViewController:(UIViewController *)viewController animated:(BOOL)animated
{
  ...
  UIViewController *currentViewController = [[HKNavigation sharedInstance] currentViewController];
